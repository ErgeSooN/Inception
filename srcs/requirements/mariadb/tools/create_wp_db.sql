...
-- Eğer "ayaman" adında bir kullanıcı henüz oluşturulmadıysa, bu komutla "ayaman" adında bir kullanıcı oluşturur.
--	Bu kullanıcıya tüm IP adreslerinden bağlantı izni verilmiştir.
CREATE USER IF NOT EXISTS 'ayaman'@'%' IDENTIFIED BY 'pass123';

-- "ayaman" kullanıcısına "wordpress" veritabanındaki tüm tablolara erişim ve değiştirme yetkisi verir.
GRANT ALL PRIVILEGES ON wordpress.* TO 'ayaman'@'%';
